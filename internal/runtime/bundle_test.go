...
func TestRuntimeBundleIncludesBoundedEvidenceAndRedactsCanaries(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(authDir, "account.json"), []byte(authCanary), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := native.Bundle(context.Background(), installation, destination); err != nil {
		t.Fatal(err)
	}
	...
	for _, file := range reader.File {
		...
		if _, err := io.CopyN(&archive, handle, 10<<20); err != nil && !errors.Is(err, io.EOF) {
			_ = handle.Close()
			t.Fatal(err)
		}
	...
	for name, present := range wanted {
		if !present {
			t.Errorf("bundle entry %q is missing", name)
		}
	}
	for _, canary := range []string{proxyCanary, managementCanary, authCanary} {
		if bytes.Contains(archive.Bytes(), []byte(canary)) {
			t.Fatalf("bundle disclosed canary %q", canary)
		}
	}
	if bytes.Contains(archive.Bytes(), []byte("account.json")) {
		t.Fatal("bundle disclosed an auth-file name")
	}
}
