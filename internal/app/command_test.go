...
func TestUnavailableUseCasesFailsClosed(t *testing.T) {
	...
	if err == nil {
		t.Fatal("unavailable composition reported success")
	}
	var typed *pmuxerr.Error
	if !errors.As(err, &typed) {
		t.Fatalf("error type = %T", err)
	}
	if typed.Code != pmuxerr.CodeDependencyMissing || typed.Class != pmuxerr.Environment {
		t.Fatalf("error = %+v", typed)
	}
	if pmuxerr.ExitCode(err) != 4 {
		t.Fatalf("exit = %d", pmuxerr.ExitCode(err))
	}
}
