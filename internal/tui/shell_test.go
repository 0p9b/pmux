...
func key(value string) tea.KeyMsg {
	if value == "enter" { return tea.KeyMsg{Type: tea.KeyEnter} }
	if value == "esc" { return tea.KeyMsg{Type: tea.KeyEsc} }
	if value == "tab" { return tea.KeyMsg{Type: tea.KeyTab} }
	if value == "up" { return tea.KeyMsg{Type: tea.KeyUp} }
	if value == "down" { return tea.KeyMsg{Type: tea.KeyDown} }
...
func update(t *testing.T, shell *Shell, msg tea.Msg) (*Shell, tea.Cmd) {
	...
	if !ok { t.Fatalf("Update returned %T, want *Shell", model) }
	return updated, cmd
}
...
func TestInitAndFirstModelRenderUseOnlyCachedState(t *testing.T) {
	...
	if cmd := shell.Init(); cmd != nil { t.Fatal("Init returned a command; startup must be cache-only") }
	shell, _ = update(t, shell, key("3"))
	view := shell.View()
	if !strings.Contains(view, "runtime-model-from-cache") { t.Fatalf("cached model missing from first model render:\n%s", view) }
	if len(facade.calls) != 0 { t.Fatalf("first render called facade %d time(s)", len(facade.calls)) }
}
...
func TestNumberKeysNavigateEveryPrimaryScreen(t *testing.T) {
	...
	for i := 1; i <= 9; i++ {
		...
		if shell.Screen() != Screen(i-1) { t.Fatalf("key %d selected %s", i, shell.Screen()) }
		if !strings.Contains(shell.View(), screenNames[i-1]) { t.Fatalf("screen %d did not render its title", i) }
	}
}
...
func TestListKeyboardFocusAndSearch(t *testing.T) {
	...
	if shell.selected[Models] != 1 { t.Fatalf("down selected %d, want 1", shell.selected[Models]) }
	shell, _ = update(t, shell, key("/"))
	shell, _ = update(t, shell, key("second"))
	if shell.SearchQuery() != "second" { t.Fatalf("query = %q", shell.SearchQuery()) }
	if strings.Contains(shell.View(), "runtime-model-from-cache") { t.Fatal("local search did not filter first model") }
	shell, _ = update(t, shell, key("esc"))
	if shell.SearchQuery() != "" { t.Fatal("first escape did not clear search") }
}
...
func TestTinyTerminalGuardIsOnlyContent(t *testing.T) {
	...
	if got := shell.View(); got != want { t.Fatalf("tiny view = %q, want %q", got, want) }
}
...
func TestPlainNoColorAndReducedMotion(t *testing.T) {
	...
	shell.busy = true
	view := shell.View()
	if strings.Contains(view, "\x1b[") { t.Fatalf("plain/no-color output contains ANSI: %q", view) }
	if strings.Contains(view, "⠋") || strings.Contains(view, "⠙") { t.Fatalf("reduced-motion output contains animated spinner: %q", view) }
	if !strings.Contains(view, "✓ Authenticated") || !strings.Contains(view, ">") { t.Fatal("status/focus must remain textual without color") }
}
...
func TestSecretsAndTerminalControlsAreNeverRendered(t *testing.T) {
	...
	for _, screen := range []Screen{Dashboard, Config, Logs} {
		...
		for _, secret := range []string{proxy, "complete-management-secret", "complete-provider-key", "complete-config-key", callbackCode, "secret-state"} {
			if strings.Contains(view, secret) { t.Fatalf("%s rendered complete secret %q:\n%s", screen, secret, view) }
		}
		if strings.Contains(view, "\x1b]") { t.Fatalf("%s rendered terminal control sequence", screen) }
	}
}
...
func TestEveryOperationalActionHasCanonicalCLIParityMetadata(t *testing.T) {
	...
	for _, action := range Actions() {
		if action.ID == "" || action.Label == "" || action.Key == "" { t.Fatalf("incomplete action metadata: %#v", action) }
		if len(action.Command) < 1 || action.Command[0] != "pmux" { t.Fatalf("action %s has noncanonical command %#v", action.ID, action.Command) }
		...
		for _, forbidden := range []string{" pmux env", " pmux keys", " pmux fleet", " pmux use", "--repair"} {
			if strings.Contains(" "+command, forbidden) { t.Fatalf("action %s exposes forbidden command %q", action.ID, command) }
		}
		if seen[action.ID] { t.Fatalf("duplicate action ID %s", action.ID) }
		seen[action.ID] = true
	}
	for screen := Dashboard; screen <= Logs; screen++ {
		if len(ActionsFor(screen)) == 0 { t.Fatalf("%s has no command parity metadata", screen) }
	}
}
...
func TestReadActionCallsOnlyInjectedFacadeAfterInput(t *testing.T) {
	...
	if cmd == nil { t.Fatal("refresh did not return a facade command") }
	if len(facade.calls) != 0 { t.Fatal("facade called synchronously from Update") }
	msg := cmd()
	if len(facade.calls) != 1 || facade.calls[0].ID != ActionDashboardRefresh { t.Fatalf("facade calls = %#v", facade.calls) }
	shell, _ = update(t, shell, msg)
	if shell.Busy() { t.Fatal("shell remained busy after facade result") }
}
...
func TestMutationRequiresExactConfirmation(t *testing.T) {
	...
	if cmd != nil || shell.pending == nil { t.Fatal("stop should open confirmation without executing") }
	shell, cmd = update(t, shell, key("wrong"))
	if cmd != nil { t.Fatal("typing wrong confirmation executed mutation") }
	shell, cmd = update(t, shell, key("enter"))
	if cmd != nil { t.Fatal("wrong phrase submitted mutation") }
	shell.confirm = "stop"
	shell, cmd = update(t, shell, key("enter"))
	if cmd == nil { t.Fatal("exact confirmation did not schedule mutation") }
	_ = cmd()
	if len(facade.calls) != 1 || facade.calls[0].ID != ActionServiceStop { t.Fatalf("unexpected calls: %#v", facade.calls) }
}
...
func TestSafeTextUsesSharedMaskShape(t *testing.T) {
	...
	if !strings.Contains(safeText(secret), masked) { t.Fatalf("safeText mask %q does not match shared mask %q", safeText(secret), masked) }
	if strings.Contains(safeText(secret), secret) { t.Fatal("safeText retained complete secret") }
}
