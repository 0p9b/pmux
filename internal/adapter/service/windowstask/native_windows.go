...
func (nativeCOM) GetTask(ctx context.Context, name string) (RegisteredTask, error) {
	...
	err := withTaskFolder(ctx, func(_ *ole.IDispatch, folder *ole.IDispatch) error {
		...
		if err != nil {
			if taskMissing(err) { return ErrTaskNotFound }
			return err
		}
		task := value.ToIDispatch()
		if task == nil { return ErrTaskNotFound }
		defer task.Release()
		...
		if err == nil { out.LastResult = int32(last) }
			...
			if instances != nil {
				...
				if count > 0 {
					...
					if itemErr == nil {
						...
						if item != nil { out.PID, _ = intProperty(item, "EnginePID"); item.Release() }
					}
				}
			}
		}
...
func (nativeCOM) RegisterTaskDefinition(ctx context.Context, definition TaskDefinition) error {
	return withTaskFolder(ctx, func(service, folder *ole.IDispatch) error {
		...
		if err != nil { return err }
		task := value.ToIDispatch()
		if task == nil { return errors.New("Task Scheduler returned no task definition") }
		defer task.Release()

		registration, err := getDispatch(task, "RegistrationInfo")
		if err != nil { return err }
		if _, err = oleutil.PutProperty(registration, "Description", definition.Description); err == nil { _, err = oleutil.PutProperty(registration, "Author", definition.Author) }
		...
		if err == nil { _, err = oleutil.PutProperty(principal, "RunLevel", 0) }
		principal.Release()
		if err != nil { return err }

		settings, err := getDispatch(task, "Settings")
		if err != nil { return err }
		for _, property := range []struct{name string; value any}{
			...
			if _, err = oleutil.PutProperty(settings, property.name, property.value); err != nil { break }
		}
		settings.Release()
		if err != nil { return err }
		...
		if definition.LogonTrigger {
			...
			if created != nil && created.ToIDispatch() != nil { created.ToIDispatch().Release() }
			triggers.Release()
		...
		if action == nil { return errors.New("Task Scheduler returned no ExecAction") }
		defer action.Release()
		if _, err = oleutil.PutProperty(action, "Path", definition.Exec.Executable); err != nil { return err }
		if _, err = oleutil.PutProperty(action, "Arguments", windows.ComposeCommandLine(definition.Exec.Arguments)); err != nil { return err }
		if _, err = oleutil.PutProperty(action, "WorkingDirectory", definition.Exec.WorkingDirectory); err != nil { return err }
...
func (nativeCOM) RunTask(ctx context.Context, name string) error {
	return withRegisteredTask(ctx, name, func(task *ole.IDispatch) error {
		...
		if value != nil && value.ToIDispatch() != nil { value.ToIDispatch().Release() }
		return err
	})
}
...
func (nativeCOM) StopTask(ctx context.Context, name string, timeout time.Duration) error {
	...
	for {
		...
		if err != nil { return err }
		if task.State != TaskStateRunning && task.State != TaskStateQueued { return nil }
		if !time.Now().Before(deadline) { return fmt.Errorf("Task Scheduler did not stop %q within %s", name, timeout) }
...
func withRegisteredTask(ctx context.Context, name string, fn func(*ole.IDispatch) error) error {
	return withTaskFolder(ctx, func(_ *ole.IDispatch, folder *ole.IDispatch) error {
		value, err := oleutil.CallMethod(folder, "GetTask", name)
		if err != nil { if taskMissing(err) { return ErrTaskNotFound }; return err }
		task := value.ToIDispatch()
		if task == nil { return ErrTaskNotFound }
		defer task.Release()
		return fn(task)
...
func withTaskFolder(ctx context.Context, fn func(*ole.IDispatch, *ole.IDispatch) error) error {
	...
	if _, err := oleutil.CallMethod(service, "Connect"); err != nil { return err }
	value, err := oleutil.CallMethod(service, "GetFolder", `\`)
	if err != nil { return err }
	folder := value.ToIDispatch()
	if folder == nil { return errors.New("Task Scheduler root folder is unavailable") }
...
func readDefinition(task *ole.IDispatch, name string) (TaskDefinition, error) {
	...
	if count < 1 { actions.Release(); return out, errors.New("Task Scheduler definition has no action") }
	value, err := oleutil.CallMethod(actions, "Item", 1)
	actions.Release()
	if err != nil { return out, err }
	action := value.ToIDispatch()
	if action == nil { return out, errors.New("Task Scheduler action is unavailable") }
	defer action.Release()
...
func getDispatch(object *ole.IDispatch, name string) (*ole.IDispatch, error) {
	...
	if dispatch == nil { return nil, fmt.Errorf("Task Scheduler property %s is unavailable", name) }
	return dispatch, nil
}
func stringProperty(object *ole.IDispatch, name string) (string, error) { value, err := oleutil.GetProperty(object, name); if err != nil { return "", err }; return value.ToString(), nil }
func intProperty(object *ole.IDispatch, name string) (int, error) { value, err := oleutil.GetProperty(object, name); if err != nil { return 0, err }; switch v := value.Value().(type) { case int: return v, nil; case int8: return int(v), nil; case int16: return int(v), nil; case int32: return int(v), nil; case int64: return int(v), nil; case uint: return int(v), nil; case uint32: return int(v), nil; default: return int(value.Val), nil } }
func boolProperty(object *ole.IDispatch, name string) (bool, error) { value, err := oleutil.GetProperty(object, name); if err != nil { return false, err }; if v, ok := value.Value().(bool); ok { return v, nil }; return value.Val != 0, nil }
func taskState(value int) TaskState { switch value { case 1: return TaskStateDisabled; case 2: return TaskStateQueued; case 3: return TaskStateReady; case 4: return TaskStateRunning; default: return TaskStateUnknown } }
func taskMissing(err error) bool { var oleErr *ole.OleError; return errors.As(err, &oleErr) && (uint32(oleErr.Code()) == 0x80070002 || uint32(oleErr.Code()) == 0x8004130f) }
func isoDuration(value time.Duration) string { if value <= 0 { return "PT0S" }; seconds := int64(value / time.Second); if seconds%60 == 0 { return fmt.Sprintf("PT%dM", seconds/60) }; return fmt.Sprintf("PT%dS", seconds) }
func parseISODuration(value string) time.Duration { var number int64; if _, err := fmt.Sscanf(strings.ToUpper(value), "PT%dM", &number); err == nil { return time.Duration(number)*time.Minute }; if _, err := fmt.Sscanf(strings.ToUpper(value), "PT%dS", &number); err == nil { return time.Duration(number)*time.Second }; return 0 }
...
func (nativeLogReader) Open(ctx context.Context, _ string, logDir string, tail int, follow bool) (io.ReadCloser, error) {
	...
	if tail > 0 {
		...
		if len(lines) > tail+1 { lines = lines[len(lines)-tail-1:] }
		body = bytes.Join(lines, []byte{'\n'})
	}
	if !follow { return io.NopCloser(bytes.NewReader(body)), nil }
	file, err := os.Open(path)
	if err != nil { return nil, err }
	if _, err := file.Seek(int64(len(body)), io.SeekStart); err != nil { file.Close(); return nil, err }
