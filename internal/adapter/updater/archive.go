...
func verifyArchiveChecksum(archivePath, checksumsPath, archiveName string) error {
	...
	closeErr := f.Close()
	if copyErr != nil {
		return pmuxerr.Wrap(copyErr, pmuxerr.InstallIntegrityFailed, pmuxerr.Environment, "Could not hash the downloaded archive.")
	}
	if closeErr != nil {
		return pmuxerr.Wrap(closeErr, pmuxerr.InstallIntegrityFailed, pmuxerr.Environment, "Could not close the downloaded archive.")
	}
...
func extractTarExecutable(archivePath, destination, executableName string) error {
	...
	for {
		...
		if found {
			return fmt.Errorf("archive contains more than one %s", executableName)
		}
		if err := writeExtracted(filepath.Join(destination, executableName), tr, h.Size); err != nil {
			return err
		}
		found = true
	}
...
func writeExtracted(destination string, source io.Reader, size int64) error {
	if size < 0 || size > defaultMaxDownload {
		return fmt.Errorf("executable has invalid size %d", size)
	}
	...
	if written != size {
		return fmt.Errorf("executable size mismatch: expected %d, wrote %d", size, written)
	}
	if syncErr != nil {
		return syncErr
	}
...
func verifyExecutable(filename string, target Target) error {
	...
	case "linux":
		...
		if err != nil {
			return unsupportedExecutable(target, err)
		}
	...
	case "windows":
		...
		case pe.IMAGE_FILE_MACHINE_ARM64:
			arch = "arm64"
		default:
			arch = fmt.Sprintf("0x%x", f.Machine)
		}
	default:
