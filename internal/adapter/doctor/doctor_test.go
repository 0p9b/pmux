...
func (s *seedSource) AbsoluteConfig(context.Context) (AbsoluteConfigFact, error) {
	return s.config, nil
}
...
func (s *seedSource) ManagementLocal(context.Context) (ManagementLocalFact, error) {
...
func TestReportExactJSONSchema(t *testing.T) {
	...
	for _, check := range checks {
		...
		for _, obsolete := range []string{"name", "category", "repairable", "applied", "verified"} {
			if _, ok := check[obsolete]; ok {
				t.Fatalf("obsolete field %q present", obsolete)
			}
		}
		var repair map[string]json.RawMessage
		if err := json.Unmarshal(check["repair"], &repair); err != nil {
			t.Fatal(err)
		}
...
func TestWarningsExitZero(t *testing.T) {
	...
	if report.Summary.Warnings != 4 || report.Summary.Failed != 0 || report.Summary.ExitCode != 0 {
...
func TestEverySeededFailureDetectedAndExitsSeven(t *testing.T) {
	...
	if report.Summary.ExitCode != 7 {
		t.Fatalf("exit = %d", report.Summary.ExitCode)
	}
	if report.Summary.Failed != 5 || report.Summary.Warnings != 1 || report.Summary.Critical != 5 {
		t.Fatalf("unexpected summary: %+v", report.Summary)
	}
	skipped := 0
	for _, check := range report.Checks {
		if check.Status == domaindoctor.StatusSkip {
			skipped++
		}
	}
	if skipped != 9 {
		t.Fatalf("skipped dependency checks = %d, want 9", skipped)
	}
}
	...
	for _, test := range cases {
		t.Run(test.id, func(t *testing.T) {
			...
			for index := range report.Checks {
				if report.Checks[index].ID == test.id {
					target = &report.Checks[index]
					break
				}
			}
			if target == nil || target.Status != test.status {
				t.Fatalf("target=%+v, want status %s", target, test.status)
			}
		})
	}
}
...
func TestRequestedCheckRunsPrerequisitesAndSkipsDependent(t *testing.T) {
	...
	if !reflect.DeepEqual(gotIDs, wantIDs) {
		t.Fatalf("check order=%v, want %v", gotIDs, wantIDs)
	}
...
func TestRunnerRedactsKnownSecretsAndControlSequences(t *testing.T) {
	...
	if string(encoded) == "" || strings.Contains(string(encoded), secret) || strings.Contains(string(encoded), "\\u001b") {
		t.Fatalf("unsafe report: %s", encoded)
	}
}
...
func (c mutableCheck) Run(context.Context) domaindoctor.CheckResult {
	if *c.pass {
		return result(domaindoctor.StatusPass, domaindoctor.SeverityCritical, "pass", nil, noRepair())
	}
	return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "fail", nil, repair("fix", "pass", false))
}

type mutableFix struct {
	id, check string
	value     *bool
	verify    bool
	events    *[]string
...
func (f *mutableFix) Apply(_ context.Context, dry bool) (domaindoctor.FixResult, error) {
	if dry {
		*f.events = append(*f.events, "preview:"+f.id)
		return domaindoctor.FixResult{CheckID: f.check, Summary: "preview"}, nil
	}
	*f.events = append(*f.events, "apply:"+f.id)
	*f.value = true
	return domaindoctor.FixResult{CheckID: f.check, Changed: true, Verified: f.verify}, nil
}
func (f *mutableFix) Rollback(context.Context) error {
	*f.events = append(*f.events, "rollback:"+f.id)
	*f.value = false
	return nil
}
...
func TestCheckRunnerIsReadOnlyEvenWhenFixRegistered(t *testing.T) {
	...
	events := []string{}
	r := NewRegistry()
	if err := r.RegisterCheck(mutableCheck{"A", &passed}); err != nil {
		t.Fatal(err)
	}
	if err := r.RegisterFix(&mutableFix{"fix-a", "A", &passed, true, &events}); err != nil {
		t.Fatal(err)
	}
	report, err := (Runner{Registry: r}).Run(context.Background())
	if err != nil {
...
func TestRunnerTopologicallyOrdersPrerequisitesAndRejectsCycles(t *testing.T) {
	...
	if err := r.RegisterCheck(child); err != nil {
		t.Fatal(err)
	}
	if err := r.RegisterCheck(parent); err != nil {
		t.Fatal(err)
	}
	if err := r.RegisterPrerequisites("CHILD", "PARENT"); err != nil {
		t.Fatal(err)
	}
...
func TestFixRunnerDryRunConfirmationAndRollback(t *testing.T) {
	...
	events := []string{}
	r := NewRegistry()
	for _, check := range []domaindoctor.Check{mutableCheck{"A", &first}, mutableCheck{"B", &second}} {
		if err := r.RegisterCheck(check); err != nil {
			t.Fatal(err)
		}
	}
	for _, fix := range []domaindoctor.Fix{&mutableFix{"fix-a", "A", &first, true, &events}, &mutableFix{"fix-b", "B", &second, false, &events}} {
		if err := r.RegisterFix(fix); err != nil {
			t.Fatal(err)
		}
	}
	...
	if first || second || len(dry.Results) != 0 || !reflect.DeepEqual(events, []string{"preview:fix-a", "preview:fix-b"}) {
		t.Fatalf("dry run mutated: first=%t second=%t events=%v", first, second, events)
	}

	events = nil
	_, err = (FixRunner{Registry: r}).Run(context.Background(), nil, FixOptions{})
	if pmuxerr.ExitCode(err) != 2 || first || second {
		t.Fatalf("missing confirmation: err=%v first=%t second=%t", err, first, second)
	}
	...
	if !out.RolledBack || first || second {
		t.Fatalf("rollback failed: %+v first=%t second=%t", out, first, second)
	}
	want := []string{"preview:fix-a", "preview:fix-b", "apply:fix-a", "apply:fix-b", "rollback:fix-b", "rollback:fix-a"}
	if !reflect.DeepEqual(events, want) {
		t.Fatalf("events = %v, want %v", events, want)
	}
	if pmuxerr.ExitCode(err) != 7 {
		t.Fatalf("verification failure exit = %d", pmuxerr.ExitCode(err))
	}
}
