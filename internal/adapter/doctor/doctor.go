...
func NewDefaultRegistry(source Source) (*Registry, error) {
	...
	for _, check := range builtins(source) {
		if err := r.RegisterCheck(check); err != nil { return nil, err }
	}
	for checkID, prerequisites := range defaultPrerequisites() {
		if err := r.RegisterPrerequisites(checkID, prerequisites...); err != nil { return nil, err }
	}
	return r, nil
}
...
func (r *Registry) RegisterPrerequisites(checkID string, prerequisites ...string) error {
	...
	for _, prerequisite := range previous { seen[prerequisite] = true }
...
func (r *Registry) Fix(checkID string) (domaindoctor.Fix, bool) { f, ok := r.fixes[checkID]; return f, ok }
...
func (r Report) JSON() ([]byte, error) {
	...
	if err != nil { return nil, pmuxerr.Wrap(err, pmuxerr.UnhandledInternal, pmuxerr.Internal, "doctor report could not be encoded") }
	return body, nil
}
...
func (r Runner) Run(ctx context.Context, ids ...string) (Report, error) {
	if r.Registry == nil { return Report{}, pmuxerr.New(pmuxerr.UnhandledInternal, pmuxerr.Internal, "doctor registry is required") }
	...
	for _, check := range checks {
		if err := ctx.Err(); err != nil { return Report{}, pmuxerr.Wrap(err, pmuxerr.UnhandledInternal, pmuxerr.Environment, "doctor was interrupted") }
...
func (r Runner) selectChecks(ids []string) ([]domaindoctor.Check, error) {
	...
	if len(roots) == 0 {
		...
		for _, check := range checks { roots = append(roots, check.ID()) }
	}
	...
	include = func(id string) error {
		...
		for _, prerequisite := range r.Registry.Prerequisites(id) {
			if err := include(prerequisite); err != nil { return err }
		}
		state[id] = 2
	...
	for _, id := range roots {
		if err := include(id); err != nil { return nil, err }
	}
	return selected, nil
}
...
func summarize(results []domaindoctor.CheckResult) Summary {
	...
	for _, result := range results {
		...
		case domaindoctor.StatusFail:
			out.Failed++
			if result.Severity == domaindoctor.SeverityCritical { out.Critical++ }
		}
	}
	if out.Failed > 0 { out.ExitCode = 7 }
	return out
}
...
func normalizeResult(check domaindoctor.Check, result domaindoctor.CheckResult, secrets ...string) domaindoctor.CheckResult {
	...
	if result.Evidence == nil { result.Evidence = []string{} }
	for i := range result.Evidence { result.Evidence[i] = redact.Known(safeText(result.Evidence[i]), secrets...) }
...
type checkFunc struct { id, title string; run func(context.Context) domaindoctor.CheckResult }
...
func builtins(source Source) []domaindoctor.Check {
	return []domaindoctor.Check{
		checkFunc{CheckBinary, "CLIProxyAPI binary", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Binary(ctx); return checkBinary(fact, err) }},
		checkFunc{CheckCompatibility, "CLIProxyAPI compatibility", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Compatibility(ctx); return checkCompatibility(fact, err) }},
		checkFunc{CheckConfigParse, "Configuration syntax", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.AbsoluteConfig(ctx); return checkConfigParse(fact, err) }},
		checkFunc{CheckAbsoluteConfig, "Absolute config and runtime", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.AbsoluteConfig(ctx); return checkAbsoluteConfig(fact, err) }},
		checkFunc{CheckWSAuth, "WebSocket authentication", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.AbsoluteConfig(ctx); return checkWSAuth(fact, err) }},
		checkFunc{CheckSafeMode, "Proxy safe mode", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.SafeMode(ctx); return checkSafeMode(fact, err) }},
		checkFunc{CheckPermissions, "Private key and state permissions", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Permissions(ctx); return checkPermissions(fact, err) }},
		checkFunc{CheckServiceDefinition, "Service definition", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Service(ctx); return checkServiceDefinition(fact, err) }},
		checkFunc{CheckService, "Service state", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Service(ctx); return checkService(fact, err) }},
		checkFunc{CheckPort, "Proxy port ownership", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Port(ctx); return checkPort(fact, err) }},
		checkFunc{CheckHealth, "Proxy health", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Health(ctx); return checkHealth(fact, err) }},
		checkFunc{CheckManagementLocal, "Local management API", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.ManagementLocal(ctx); return checkManagementLocal(fact, err) }},
		checkFunc{CheckExposure, "Proxy network exposure", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Exposure(ctx); return checkExposure(fact, err) }},
		checkFunc{CheckProviders, "Provider status", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Providers(ctx); return checkProviders(fact, err) }},
		checkFunc{CheckAuthPermissions, "Authentication file permissions", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Permissions(ctx); return checkAuthPermissions(fact, err) }},
		checkFunc{CheckModels, "Model catalog", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Models(ctx); return checkModels(fact, err) }},
		checkFunc{CheckClaude, "Claude Code", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.Claude(ctx); return checkClaude(fact, err) }},
		checkFunc{CheckStateLock, "PMux mutation lock", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.StateLock(ctx); return checkStateLock(fact, err) }},
		checkFunc{CheckNoEgress, "No automatic update egress", func(ctx context.Context) domaindoctor.CheckResult { fact, err := source.UpdateState(ctx); return checkNoEgress(fact, err) }},
	}
}
...
func result(status domaindoctor.Status, severity domaindoctor.Severity, summary string, evidence []string, repair domaindoctor.Repair) domaindoctor.CheckResult {
	if evidence == nil { evidence = []string{} }
...
func repair(description, verification string, destructive bool) domaindoctor.Repair { return domaindoctor.Repair{Available: true, Description: description, Destructive: destructive, ConfirmationRequired: true, Verification: verification} }
func guidance(description, verification string) domaindoctor.Repair { return domaindoctor.Repair{Available: false, Description: description, Destructive: false, ConfirmationRequired: false, Verification: verification} }
...
func checkBinary(f BinaryFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("CLIProxyAPI binary", err, domaindoctor.SeverityCritical) }
	if f.NotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Native binary check is not applicable", []string{f.NotApplicable}, noRepair()) }
	e := []string{"path: " + f.Path}
	if f.Version != "" { e = append(e, "version: "+f.Version) }
	if !f.Exists { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI binary is missing", e, noRepair()) }
	if !f.Executable { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI binary is not executable", e, noRepair()) }
	if !f.ArchitectureOK { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI binary architecture does not match this host", e, noRepair()) }
	if !filepath.IsAbs(f.Path) { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI binary path is not absolute", e, noRepair()) }
	if f.Managed && !f.ChecksumOK { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Managed CLIProxyAPI binary checksum does not match", e, noRepair()) }
...
func checkConfigParse(f AbsoluteConfigFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("configuration syntax", err, domaindoctor.SeverityCritical) }
	evidence := []string{"config: " + f.ConfigPath}
	if f.ParseDetail != "" { evidence = append(evidence, f.ParseDetail) }
	if !f.ConfigReadable { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Configuration file is not readable", evidence, noRepair()) }
	if !f.ConfigParsed { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Configuration file is invalid", evidence, noRepair()) }
...
func checkWSAuth(f AbsoluteConfigFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("WebSocket authentication", err, domaindoctor.SeverityWarning) }
...
func checkAbsoluteConfig(f AbsoluteConfigFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("absolute config", err, domaindoctor.SeverityCritical) }
	if f.ProcessContractNotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Native service process contract is not applicable", []string{f.ProcessContractNotApplicable}, noRepair()) }
	e := []string{"config: " + f.ConfigPath, "runtime: " + f.RuntimeDir}
	if !filepath.IsAbs(f.ConfigPath) || !f.ArgvUsesAbsolutePath { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Service does not use an absolute config path", e, guidance("run the separately confirmed adoption hardening transaction to rewrite the service definition", "service argv and health re-check")) }
	if !filepath.IsAbs(f.RuntimeDir) { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Service runtime directory is not absolute", e, guidance("run the separately confirmed adoption hardening transaction to install the canonical runtime directory", "service runtime directory and health re-check")) }
	if f.RuntimeContainsDotEnv { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Service runtime directory contains .env", e, guidance("remove the unexpected .env after inspecting it; PMux will not delete an unowned file automatically", "runtime directory inspection and health re-check")) }
	if len(f.StoreOverrides) > 0 { sort.Strings(f.StoreOverrides); e = append(e, "store overrides: "+strings.Join(f.StoreOverrides, ", ")); return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Service environment can override the recorded config", e, guidance("run the separately confirmed adoption hardening transaction to install a scrubbed service environment", "service environment and health re-check")) }
...
func checkSafeMode(f SafeModeFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("proxy safe mode", err, domaindoctor.SeverityCritical) }
	e := []string{fmt.Sprintf("http status: %d", f.HTTPStatus)}
	if f.Header != "" { e = append(e, "X-Cpa-Safe-Mode: "+f.Header) }
	if f.PlaceholderConfigured || f.Header != "" { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI is in safe mode because the configured proxy key is a placeholder", e, repair("back up config, generate a private random proxy key, and remove confirmed placeholders", "authenticated /v1/models returns 200 without X-Cpa-Safe-Mode", true)) }
	if !f.Authenticated { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Proxy authentication did not succeed", e, noRepair()) }
...
func permissionResult(f PermissionsFact, auth bool) domaindoctor.CheckResult {
	...
	if auth && f.AuthNotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Authentication-path permission check is not applicable", []string{f.AuthNotApplicable}, noRepair()) }
	if !auth && f.SecretNotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Native secret-path permission check is not applicable", []string{f.SecretNotApplicable}, noRepair()) }
	for _, target := range f.Targets {
		if target.Auth == auth { targets = append(targets, target) }
	}
	scope := "Secret-bearing"
	if auth { scope = "Authentication" }
	if len(targets) == 0 {
	...
	for _, target := range targets {
		if !target.Secure { insecure = append(insecure, target.Path+": "+target.Detail) }
	}
	if len(insecure) > 0 {
...
func checkAuthPermissions(f PermissionsFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("authentication file permissions", err, domaindoctor.SeverityCritical) }
	return permissionResult(f, true)
}

func checkPermissions(f PermissionsFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("private permissions", err, domaindoctor.SeverityCritical) }
	return permissionResult(f, false)
}

func checkServiceDefinition(f ServiceFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("service definition", err, domaindoctor.SeverityWarning) }
	if f.ExternallyManaged { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Native service definition is externally managed", []string{f.Detail}, noRepair()) }
	evidence := []string{"backend: " + f.Backend}
	if f.Detail != "" { evidence = append(evidence, f.Detail) }
	if !f.Required {
	...
	if !f.Installed { return result(domaindoctor.StatusFail, domaindoctor.SeverityWarning, "Selected native service definition is not installed", evidence, noRepair()) }
	if !f.DefinitionOwned { return result(domaindoctor.StatusFail, domaindoctor.SeverityWarning, "Selected service definition is not PMux-owned", evidence, noRepair()) }
	if !f.IdentityMatches { return result(domaindoctor.StatusFail, domaindoctor.SeverityWarning, "Selected service identity is not canonical", evidence, noRepair()) }
...
func checkService(f ServiceFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("service state", err, domaindoctor.SeverityCritical) }
	e := []string{"backend: " + f.Backend}
	if f.ExternallyManaged {
		evidence := []string{"backend: " + f.Backend, f.Detail}
		if !f.Running { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Externally managed CLIProxyAPI container is not running", evidence, noRepair()) }
	...
	if f.Detail != "" { e = append(e, f.Detail) }
	if f.Required && !f.Installed { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Selected service is not installed", e, guidance("install the canonical service with `pmux service install`", "native service status and absolute argv re-check")) }
	if f.CrashLoop { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI service is crash-looping", e, noRepair()) }
	if !f.Running { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI service is not running", e, guidance("start the selected service with `pmux service start`", "service reaches running and passes health check")) }
...
func checkPort(f PortFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("proxy port ownership", err, domaindoctor.SeverityCritical) }
	evidence := []string{fmt.Sprintf("listener: %s:%d", valueOr(f.Host, "unknown"), f.Port)}
	if f.Owner != "" { evidence = append(evidence, "owner: "+f.Owner) }
	if !f.Listening { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Recorded proxy port is not listening", evidence, noRepair()) }
	if !f.ExpectedOwner {
...
func checkHealth(f HealthFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("proxy health", err, domaindoctor.SeverityCritical) }
	e := []string{fmt.Sprintf("http status: %d", f.HTTPStatus)}
	if f.Endpoint != "" { e = append(e, "endpoint: "+f.Endpoint) }
	if f.LatencyMS > 0 { e = append(e, fmt.Sprintf("latency_ms: %d", f.LatencyMS)) }
	if f.HTTPStatus != 200 { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI health check failed", e, noRepair()) }
	if f.Version == "" { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "CLIProxyAPI is healthy, but its version is unknown", e, noRepair()) }
...
func checkManagementLocal(f ManagementLocalFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("local management API", err, domaindoctor.SeverityCritical) }
	if f.NotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Management ownership check is externally managed", []string{f.NotApplicable}, noRepair()) }
	...
	if f.Required && !f.Enabled { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Required local management API is not enabled", evidence, noRepair()) }
	if f.Enabled && !f.Authenticated { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Local management API authentication failed", evidence, noRepair()) }
	if f.AllowRemote { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Management API allows remote access", evidence, noRepair()) }
...
func checkExposure(f ExposureFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("proxy network exposure", err, domaindoctor.SeverityCritical) }
	...
	if f.Loopback {
		if !f.ManagementLocal { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Proxy is loopback-bound, but management is remotely exposed", evidence, noRepair()) }
...
func checkProviders(f ProviderFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("provider status", err, domaindoctor.SeverityWarning) }
	if f.NotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Provider inventory is unavailable for this external container", []string{f.NotApplicable}, noRepair()) }
	e := []string{fmt.Sprintf("configured: %d", f.Configured), fmt.Sprintf("usable: %d", f.Usable), fmt.Sprintf("unavailable: %d", f.Unavailable)}
	if f.Detail != "" { e = append(e, f.Detail) }
	if f.Configured == 0 { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "No provider credentials are configured", e, noRepair()) }
	if f.Usable == 0 { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "No provider credential is currently usable", e, noRepair()) }
	if f.Unavailable > 0 { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "Some provider credentials are unavailable", e, noRepair()) }
...
func checkModels(f ModelsFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("model catalog", err, domaindoctor.SeverityWarning) }
	if f.NotApplicable != "" { return result(domaindoctor.StatusSkip, domaindoctor.SeverityInfo, "Model inventory is unavailable for this external container", []string{f.NotApplicable}, noRepair()) }
	e := []string{"source: " + f.Source, fmt.Sprintf("models: %d", f.Count)}
	if f.Cause != "" { e = append(e, f.Cause) }
	if !f.DiscoverySucceeded { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "Live model discovery did not succeed", e, noRepair()) }
	if f.Count == 0 { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "No models are currently available", e, noRepair()) }
...
func checkClaude(f ClaudeFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("Claude Code", err, domaindoctor.SeverityWarning) }
	e := []string{}
	if f.Path != "" { e = append(e, "path: "+f.Path) }
	if f.Version != "" { e = append(e, "version: "+f.Version) }
	if !f.Found { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "Claude Code was not found", e, noRepair()) }
	if !f.VersionKnown { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "Claude Code version is unknown", e, noRepair()) }
	if !f.Supported { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "Claude Code is older than the supported v2 floor", e, noRepair()) }
...
func checkCompatibility(f CompatibilityFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("CLIProxyAPI compatibility", err, domaindoctor.SeverityCritical) }
	e := []string{"detected: " + valueOr(f.DetectedVersion, "unknown"), "minimum: " + valueOr(f.MinimumVersion, "unknown")}
	if !f.VersionKnown { return result(domaindoctor.StatusWarn, domaindoctor.SeverityWarning, "CLIProxyAPI version is unknown; feature probes remain required", e, noRepair()) }
	if !f.FloorSatisfied { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI is below PMux's supported management floor", e, noRepair()) }
	if len(f.MissingCapabilities) > 0 { sort.Strings(f.MissingCapabilities); e = append(e, "missing: "+strings.Join(f.MissingCapabilities, ", ")); return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "CLIProxyAPI is missing required capabilities", e, noRepair()) }
...
func checkNoEgress(f UpdateStateFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("automatic update state", err, domaindoctor.SeverityCritical) }
	...
	if f.AutomaticChecksEnabled || f.UnexpectedEgress { return result(domaindoctor.StatusFail, domaindoctor.SeverityCritical, "Automatic update activity or unrequested egress is enabled", e, guidance("disable update checks with `pmux config --scope pmux set update.check false`", "ordinary doctor performs no non-loopback request")) }
	if f.LastCheckExplicit { e = append(e, "last update check: explicit") }
...
func checkStateLock(f StateLockFact, err error) domaindoctor.CheckResult {
	if err != nil { return probeFailure("PMux mutation lock", err, domaindoctor.SeverityInfo) }
	...
	if f.Holder != "" { evidence = append(evidence, "holder: "+f.Holder) }
	if !f.ReadOnlyStateAccessible { return result(domaindoctor.StatusFail, domaindoctor.SeverityInfo, "PMux state cannot be inspected read-only", evidence, noRepair()) }
	if f.MutationRequested && !f.MutationLockAvailable { return result(domaindoctor.StatusWarn, domaindoctor.SeverityInfo, "Another PMux mutation currently holds the state lock", evidence, noRepair()) }
...
func safeText(value string) string {
	...
	out.Grow(len(value))
	for _, r := range value {
		if r == '\n' || r == '\t' || (unicode.IsPrint(r) && r != '\x1b') { out.WriteRune(r) }
	}
	return out.String()
}
