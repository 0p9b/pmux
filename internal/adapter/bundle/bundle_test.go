...
func TestBundleIsPrivateRedactedAndExcludesAuthContents(t *testing.T) {
	...
	if _, ok := files["auth/account.json"]; ok {
		t.Fatal("auth file was included")
	}
	if _, ok := files["renamed-auth.txt"]; ok {
		t.Fatal("kind-marked auth file was included")
	}
	for name, data := range files {
		if bytes.Contains(data, []byte(canary)) {
			t.Fatalf("canary in %s", name)
		}
		if bytes.Contains(data, []byte("AUTH-CONTENT-DO-NOT-INCLUDE")) {
			t.Fatalf("auth content in %s", name)
		}
		if bytes.Contains(data, []byte("sk-abcdefghijklmnop")) {
			t.Fatalf("proxy key in %s", name)
		}
		if bytes.Contains(data, []byte(bareSecret)) {
			t.Fatalf("unseeded structured secret in %s", name)
		}
		if bytes.Contains(data, []byte("canary-private-material")) {
			t.Fatalf("PEM private key in %s", name)
		}
	}
	if !bytes.Contains(files["doctor.json"], []byte("<redacted>")) {
		t.Fatalf("doctor was not redacted: %s", files["doctor.json"])
	}
...
func TestBundleUsesPermissionAndVerificationHooks(t *testing.T) {
	...
	if _, err := builder.Build(t.Context(), destination, []Entry{{ArchivePath: "doctor.json", Kind: KindJSON, Data: []byte(`{"ok":true}`)}}); err != nil {
		t.Fatal(err)
	}
	if !secureCalled || !verifyCalled {
		t.Fatalf("hooks not called: secure=%t verify=%t", secureCalled, verifyCalled)
	}
}
...
func TestBundleRefusesExistingDestinationAndUnsafePaths(t *testing.T) {
	...
	if _, statErr := os.Stat(unsafe); !os.IsNotExist(statErr) {
		t.Fatalf("unsafe archive was created: %v", statErr)
	}
}
...
func TestBundleRejectsCanaryInArchiveFilename(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected filename canary rejection")
	}
	if _, statErr := os.Stat(destination); !os.IsNotExist(statErr) {
		t.Fatalf("archive was created: %v", statErr)
	}
}
