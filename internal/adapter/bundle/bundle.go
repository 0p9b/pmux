...
func (b Builder) Build(ctx context.Context, destination string, entries []Entry) (result Result, retErr error) {
	if err := ctx.Err(); err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.CodeInterrupted, pmuxerr.Environment, "diagnostic bundle creation was interrupted") }
	absolute, err := filepath.Abs(destination)
	if err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigValidationFailed, pmuxerr.Environment, "diagnostic bundle destination is invalid") }
	if err := os.MkdirAll(filepath.Dir(absolute), 0o700); err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigUnreadable, pmuxerr.Environment, "diagnostic bundle directory could not be created") }

	now := time.Now().UTC()
	if b.Now != nil { now = b.Now().UTC() }
	manifest := Manifest{SchemaVersion: 1, CreatedAt: now, Entries: []ManifestEntry{}, Excluded: map[string]int{}}
	type stagedEntry struct { name string; kind Kind; data []byte }
	staged := make([]stagedEntry, 0, len(entries)+1)
	seen := make(map[string]bool)
	for _, entry := range entries {
		if err := ctx.Err(); err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.CodeInterrupted, pmuxerr.Environment, "diagnostic bundle creation was interrupted") }
		reason := b.exclusionReason(entry)
		if reason != "" { manifest.Excluded[reason]++; continue }
		name, nameErr := cleanArchivePath(entry.ArchivePath)
		if nameErr != nil { return Result{}, nameErr }
		...
		if name == "MANIFEST.json" || seen[name] { return Result{}, pmuxerr.New(pmuxerr.ConfigValidationFailed, pmuxerr.User, "diagnostic bundle contains a duplicate archive path") }
		seen[name] = true
	...
	if err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.UnhandledInternal, pmuxerr.Internal, "diagnostic bundle manifest could not be encoded") }
	manifestBytes = append(manifestBytes, '\n')
	if secret := b.firstCanary(manifestBytes); secret != "" { return Result{}, pmuxerr.New(pmuxerr.UnhandledInternal, pmuxerr.Internal, "diagnostic bundle manifest contains a known secret") }
	...
	if err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigMutationConflict, pmuxerr.Environment, "diagnostic bundle destination already exists or cannot be created") }
	committed := false
	defer func() {
		if !committed { _ = file.Close(); _ = os.Remove(absolute) }
	}()
	...
	for _, entry := range staged {
		...
		if createErr != nil { return Result{}, pmuxerr.Wrap(createErr, pmuxerr.UnhandledInternal, pmuxerr.Internal, "diagnostic bundle entry could not be created") }
		if _, writeErr := writer.Write(entry.data); writeErr != nil { return Result{}, pmuxerr.Wrap(writeErr, pmuxerr.ConfigUnreadable, pmuxerr.Environment, "diagnostic bundle entry could not be written") }
	}
	if err := zw.Close(); err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigUnreadable, pmuxerr.Environment, "diagnostic bundle archive could not be finalized") }
	if err := file.Sync(); err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigUnreadable, pmuxerr.Environment, "diagnostic bundle archive could not be flushed") }
	if err := file.Close(); err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigUnreadable, pmuxerr.Environment, "diagnostic bundle archive could not be closed") }
	...
	if err != nil { return Result{}, pmuxerr.Wrap(err, pmuxerr.ConfigUnreadable, pmuxerr.Environment, "diagnostic bundle archive could not be verified") }
	if err := b.verifyArchive(archiveBytes); err != nil { return Result{}, err }
...
func (b Builder) exclusionReason(entry Entry) string {
	if entry.Kind == KindAuthFile { return "auth-file-content" }
	...
	if entry.SourcePath != "" {
		for _, root := range b.AuthRoots {
			if within(root, entry.SourcePath) { return "auth-file-content" }
		}
		base := strings.ToLower(filepath.Base(entry.SourcePath))
		if base == "api-key.txt" || base == "secrets.json" { return "secret-store" }
		if strings.Contains(filepath.ToSlash(entry.SourcePath), "/backups/") { return "secret-bearing-backup" }
	}
	return ""
}
...
func within(root, path string) bool {
	...
	if rootErr != nil || pathErr != nil { return false }
...
func sanitizeControls(value string) string {
	...
	out.Grow(len(value))
	for _, r := range value {
		if r == '\n' || r == '\r' || r == '\t' || (unicode.IsPrint(r) && r != '\x1b') { out.WriteRune(r) }
	}
	return out.String()
}

func (b Builder) firstCanary(data []byte) string {
	for _, secret := range b.KnownSecrets { if secret != "" && bytes.Contains(data, []byte(secret)) { return secret } }
	return ""
}

func (b Builder) verifyArchive(data []byte) error {
	reader, err := zip.NewReader(bytes.NewReader(data), int64(len(data)))
	if err != nil { return pmuxerr.Wrap(err, pmuxerr.InstallIntegrityFailed, pmuxerr.Internal, "diagnostic bundle archive verification failed") }
	for _, file := range reader.File {
		if file.FileInfo().IsDir() { continue }
		rc, openErr := file.Open()
		if openErr != nil { return pmuxerr.Wrap(openErr, pmuxerr.InstallIntegrityFailed, pmuxerr.Internal, "diagnostic bundle entry verification failed") }
		entryData, readErr := io.ReadAll(rc)
		closeErr := rc.Close()
		if readErr != nil { return pmuxerr.Wrap(readErr, pmuxerr.InstallIntegrityFailed, pmuxerr.Internal, "diagnostic bundle entry verification failed") }
		if closeErr != nil { return pmuxerr.Wrap(closeErr, pmuxerr.InstallIntegrityFailed, pmuxerr.Internal, "diagnostic bundle entry verification failed") }
		if secret := b.firstCanary(entryData); secret != "" { return pmuxerr.New(pmuxerr.UnhandledInternal, pmuxerr.Internal, fmt.Sprintf("diagnostic bundle entry %q contains a known secret", file.Name)) }
	}
	return nil
}
