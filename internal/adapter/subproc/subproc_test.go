...
func TestScrubbedEnvironmentIsAllowlist(t *testing.T) {
	...
	for _, forbidden := range []string{"secret", "PGSTORE_", "OBJECTSTORE_", "GITSTORE_", "ANTHROPIC_", "OPENAI_", "GEMINI_", "MANAGEMENT_PASSWORD", "PMUX_", "inherited.invalid"} {
		if strings.Contains(joined, forbidden) { t.Fatalf("environment leaked %q:\n%s", forbidden, joined) }
	}
	for _, expected := range []string{"HOME=/home/test", "PATH=/bin", "HTTPS_PROXY=http://configured.test", "NO_PROXY=127.0.0.1", "SSL_CERT_FILE=/private/ca.pem"} {
		if !strings.Contains(joined, expected) { t.Fatalf("environment missing %q:\n%s", expected, joined) }
	}
}
...
func TestBuildInvocationAlwaysUsesAbsoluteConfigAndCleanRuntime(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if !filepath.IsAbs(invocation.Path) || !filepath.IsAbs(invocation.Dir) { t.Fatalf("paths are not absolute: %#v", invocation) }
...
func (e *inspectingExecutor) Execute(_ context.Context, invocation Invocation, observe func(string) bool) error {
	...
	if err != nil { e.t.Fatal(err) }
	if len(entries) != 0 { e.t.Fatalf("probe auth dir is not empty: %#v", entries) }
...
func TestVersionProbeUsesGeneratedIsolatedState(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if info.Version != "7.2.92" || info.Commit != "abc123" { t.Fatalf("unexpected version: %#v", info) }
	...
	for _, required := range []string{"host: 127.0.0.1", "ws-auth: true", "allow-remote: false", "sk-"} {
		if !strings.Contains(executor.config, required) { t.Fatalf("probe config missing %q:\n%s", required, executor.config) }
	}
	if executor.port <= 0 { t.Fatalf("probe did not allocate a free loopback port: %d", executor.port) }
	if filepath.Dir(executor.authDir) != executor.invocation.Dir { t.Fatalf("auth dir not isolated under runtime: %s vs %s", executor.authDir, executor.invocation.Dir) }
	if _, err := os.Stat(executor.invocation.Dir); !os.IsNotExist(err) { t.Fatalf("probe temp directory was not removed: %v", err) }
	if after, _ := os.ReadFile(userConfig); string(after) != string(before) { t.Fatal("user config changed during isolated probe") }
	joinedEnv := strings.Join(executor.invocation.Env, "\n")
	if strings.Contains(joinedEnv, "forbidden") { t.Fatalf("probe leaked parent secrets: %s", joinedEnv) }
}
...
func TestVersionProbeUnsafeBinaryFailsWithoutExecution(t *testing.T) {
	...
	if !errors.Is(err, ErrUnsafeProbe) { t.Fatalf("expected unsafe probe, got %v", err) }
	if executor.invocation.Path != "" { t.Fatal("unsafe binary was executed") }
}
...
func mustWrite(t *testing.T, path, body string, mode os.FileMode) {
	t.Helper()
	if err := os.WriteFile(path, []byte(body), mode); err != nil { t.Fatal(err) }
