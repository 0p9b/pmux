...
func TestDockerEndpointSelectionPolicy(t *testing.T) {
	...
	for _, test := range cases {
		...
		if len(got) != len(test.want) { t.Fatalf("%s endpoints: got %#v want %#v", test.goos, got, test.want) }
		for index := range got {
			if got[index] != test.want[index] { t.Fatalf("%s endpoint %d: got %#v want %#v", test.goos, index, got[index], test.want[index]) }
		}
	}
	if got := dockerEndpointCandidates("freebsd", home); got != nil { t.Fatalf("unsupported OS selected Docker transport: %#v", got) }
}
...
func TestDockerEndpointAbsenceIsSilentButExistingEndpointErrorsSurface(t *testing.T) {
	...
	if err != nil || len(containers) != 0 { t.Fatalf("missing Docker endpoint was not silent: containers=%#v err=%v", containers, err) }
	...
	if err == nil || !strings.Contains(err.Error(), "Docker is unavailable") { t.Fatalf("existing/unusable Docker endpoint error was hidden: %v", err) }
}
