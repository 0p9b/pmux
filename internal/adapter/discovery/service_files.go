...
func (e LocalServiceEnumerator) Services(ctx context.Context) ([]ServiceEvidence, error) {
	...
	for _, path := range paths {
		...
		if os.IsNotExist(err) || os.IsPermission(err) { continue }
		if err != nil {
		...
		if info.Mode().IsRegular() {
			if serviceEvidence, ok := readServiceDefinition(path, goos); ok { results = append(results, serviceEvidence) }
			continue
		}
		...
		for _, entry := range entries {
			...
			if goos == "linux" && !strings.HasSuffix(lower, ".service") { continue }
			if goos == "darwin" && !strings.HasSuffix(lower, ".plist") { continue }
			definition := filepath.Join(path, entry.Name())
			if serviceEvidence, ok := readServiceDefinition(definition, goos); ok { results = append(results, serviceEvidence) }
		}
	}
	return results, nil
}
...
func readServiceDefinition(path, goos string) (ServiceEvidence, bool) {
	...
	if err != nil { return ServiceEvidence{}, false }
	defer file.Close()
	if goos == "darwin" { return readLaunchd(path, file) }
...
func readSystemd(path string, input io.Reader) (ServiceEvidence, bool) {
	...
	for scanner.Scan() {
		...
		case "ExecStart":
			...
			if len(evidence.Argv) > 0 { evidence.Executable = evidence.Argv[0] }
		case "WorkingDirectory":
			evidence.WorkingDir = strings.TrimSpace(value)
		}
	}
	if !looksLikeCore(evidence.Executable, evidence.Argv) && !strings.Contains(strings.ToLower(evidence.Identity), "cliproxy") { return ServiceEvidence{}, false }
...
func readLaunchd(path string, input io.Reader) (ServiceEvidence, bool) {
	...
	for {
		...
		case xml.StartElement:
			if value.Name.Local == "array" && currentKey == "ProgramArguments" { inArguments = true }
			if value.Name.Local != "key" && value.Name.Local != "string" { continue }
			var text string
			if decoder.DecodeElement(&text, &value) != nil { continue }
			if value.Name.Local == "key" { currentKey = text; continue }
			if inArguments {
				evidence.Argv = append(evidence.Argv, text)
			} else {
				switch currentKey { case "Label": evidence.Identity = text; case "WorkingDirectory": evidence.WorkingDir = text }
			}
		case xml.EndElement:
			if value.Name.Local == "array" { inArguments = false; currentKey = "" }
		}
	}
	if len(evidence.Argv) > 0 { evidence.Executable = evidence.Argv[0] }
	if !looksLikeCore(evidence.Executable, evidence.Argv) && !strings.Contains(strings.ToLower(evidence.Identity), "cliproxy") { return ServiceEvidence{}, false }
...
func splitQuoted(input string) []string {
	...
	var quote rune
	escaped := false
	flush := func() { if current.Len() > 0 { values = append(values, current.String()); current.Reset() } }
	for _, char := range input {
		if escaped { current.WriteRune(char); escaped = false; continue }
		if char == '\\' { escaped = true; continue }
		if quote != 0 {
			if char == quote { quote = 0 } else { current.WriteRune(char) }
			continue
		}
