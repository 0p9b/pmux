...
func TestWindowsDockerTransportIsBoundedNativePipe(t *testing.T) {
	...
	if !ok {
		t.Fatalf("unexpected Windows Docker enumerator: %T", newLocalContainerEnumerator())
	}
	if enumerator.Client == nil || enumerator.Client.Timeout != 2*time.Second {
		t.Fatalf("Windows Docker client is not bounded: %#v", enumerator.Client)
	}
	transport, ok := enumerator.Client.Transport.(*http.Transport)
	if !ok || transport.DialContext == nil {
		t.Fatalf("Windows Docker client has no native dialer: %#v", enumerator.Client.Transport)
	}
