...
func TestDiscoveryIsReadOnlyForFilesAndProcesses(t *testing.T) {
	...
	if err := command.Start(); err != nil { t.Fatal(err) }
	...
	for _, candidate := range candidates {
		if candidate.Process != nil && candidate.Process.PID == command.Process.Pid {
			found = true
			if candidate.Process.ConfigPath != config { t.Fatalf("wrong process config: %q", candidate.Process.ConfigPath) }
		}
	}
	if !found { t.Fatalf("live CLIProxyAPI-shaped process %d was not discovered: %#v", command.Process.Pid, candidates) }

	if _, err := os.Stat(filepath.Join(root, "anything-created-by-discovery")); !os.IsNotExist(err) { t.Fatalf("unexpected artifact: %v", err) }
	...
	if err != nil || len(procState) == 0 { t.Fatalf("discovery changed or terminated the source process: %v", err) }
}
...
type metadataStub struct { version VersionEvidence; ok bool; calls int }
func (m *metadataStub) Version(context.Context, string) (VersionEvidence, bool, error) { m.calls++; return m.version, m.ok, nil }
type proberStub struct { version ProbeVersion; err error; calls int }
func (p *proberStub) Probe(context.Context, string) (ProbeVersion, error) { p.calls++; return p.version, p.err }
...
func TestVersionDetectionOrder(t *testing.T) {
	...
	if running.Source != VersionRunningHeader || running.Version != "7.2.92" { t.Fatalf("running header did not win: %#v", running) }
	if metadata.calls != 0 || probe.calls != 0 { t.Fatal("lower-precedence detectors ran after the header succeeded") }

	fromMetadata := detector.Detect(context.Background(), Candidate{Binary: binary})
	if fromMetadata.Source != VersionMetadata || fromMetadata.Version != "7.2.91" { t.Fatalf("metadata did not win: %#v", fromMetadata) }
	if metadata.calls != 1 || probe.calls != 0 { t.Fatal("probe ran after metadata succeeded") }

	metadata.ok = false
	fromProbe := detector.Detect(context.Background(), Candidate{Binary: binary})
	if fromProbe.Source != VersionIsolatedProbe || fromProbe.Version != "7.2.90" { t.Fatalf("probe did not run: %#v", fromProbe) }
}
...
func TestUnsafeVersionProbeRecordsUnknown(t *testing.T) {
	...
	if version.Source != VersionUnknown || version.Version != "unknown" { t.Fatalf("unsafe probe was not recorded unknown: %#v", version) }
	containerVersion := (VersionDetector{Probe: probe}).Detect(context.Background(), Candidate{Container: &ContainerEvidence{ID: "c"}})
	if containerVersion.Source != VersionUnknown || probe.calls != 1 { t.Fatalf("container unexpectedly probed local binary: %#v calls=%d", containerVersion, probe.calls) }
}

type containerEnumeratorStub struct { values []ContainerEvidence }
func (s containerEnumeratorStub) Containers(context.Context) ([]ContainerEvidence, error) { return s.values, nil }
...
func TestDockerDiscoveryOnlyUsesReadOnlyInventoryAndLifecycleUnavailable(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(containers) != 1 || containers[0].ID != "abc" { t.Fatalf("unexpected containers: %#v", containers) }
	if len(transport.methods) != 1 || transport.methods[0] != "GET /v1.41/containers/json" { t.Fatalf("Docker discovery made a mutation request: %#v", transport.methods) }
	...
	if err != nil { t.Fatal(err) }
	if len(candidates) != 1 || candidates[0].Container == nil || !candidates[0].ToInstallation("docker").Container { t.Fatalf("container adoption evidence missing: %#v", candidates) }
	if !strings.Contains(strings.Join(candidates[0].Findings, " "), "externally managed") { t.Fatalf("container ownership finding missing: %#v", candidates[0].Findings) }
...
func TestServiceDefinitionDiscoveryIsReadOnlyAndFindsAbsoluteConfig(t *testing.T) {
	...
	if err := os.Mkdir(runtimeDir, 0o700); err != nil { t.Fatal(err) }
	unit := filepath.Join(root, "cliproxyapi.service")
	unitBody := "[Service]\nWorkingDirectory="+runtimeDir+"\nExecStart="+binary+" -config "+config+"\n"
	if err := os.WriteFile(unit, []byte(unitBody), 0o600); err != nil { t.Fatal(err) }
	before, _ := os.ReadFile(unit)
	services, err := (LocalServiceEnumerator{Paths: []string{unit}, GOOS: "linux"}).Services(context.Background())
	if err != nil { t.Fatal(err) }
	if len(services) != 1 || services[0].ConfigPath != config || services[0].Executable != binary { t.Fatalf("unexpected service evidence: %#v", services) }
	after, _ := os.ReadFile(unit)
	if string(before) != string(after) { t.Fatal("service discovery changed the definition") }
}
