...
func providersCommand(d dispatcher) *cobra.Command {
	...
	parent.RunE = func(cmd *cobra.Command, _ []string) error {
		...
		if d.interactive() { op = app.OpTUIProviders }
		if d.interactive() { return d.run(cmd, op, nil, nil) }
...
func providerListCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error {
		if enabled != "" && enabled != "true" && enabled != "false" { return usage("--enabled must be true or false") }
...
func providerLoginCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		if method != "auto" && method != "browser" && method != "device" { return usage("--method must be auto, browser, or device") }
...
func providerVerifyCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { return d.run(cmd, "providers.verify", args, opts("account", account, "refresh_models", refresh)) }
	return cmd
}
...
func providerToggleCommand(d dispatcher, action string) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { return d.run(cmd, app.Operation("providers."+action), args, nil) }
	return cmd
}
...
func providerRemoveCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { return d.run(cmd, "providers.remove", args, opts("keep_credentials", keep)) }
	return cmd
}
...
func modelsCommand(d dispatcher) *cobra.Command {
	...
	parent.RunE = func(cmd *cobra.Command, _ []string) error {
		op := app.OpModelsList; if d.interactive() { op = app.OpTUIModels }
		if d.interactive() { return d.run(cmd, op, nil, nil) }
...
func modelListCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { return d.run(cmd, "models.list", nil, opts("provider", provider, "available", available, "favorite", favorite, "search", search, "refresh", refresh)) }
	return cmd
}
...
func modelTestCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { return d.run(cmd, "models.test", args, opts("timeout", timeout, "provider", provider)) }
	return cmd
}
...
func modelFavoriteCommand(d dispatcher, action string) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { return d.run(cmd, app.Operation("models."+action), args, nil) }
	return cmd
}
...
func launchCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		if client != "" && client != "claude" { return usage("--client must be claude") }
		if !d.interactive() && client == "" { return usage("Noninteractive launch requires --client; no client was started.") }
		if !d.interactive() && model == "" { return usage("Noninteractive launch requires --model; no client was started.") }
		if len(args) > 0 && cmd.ArgsLenAtDash() < 0 { return usage("client arguments must follow --") }
		if hasClientModelArg(args) { return usage("Client arguments must not contain '--model'; use 'pmux launch --model <id>'.") }
...
func claudeCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		if len(args) == 0 { return usage("pmux claude requires an exact model ID") }
		if dash < 0 && len(args) != 1 { return usage("Claude arguments must follow --") }
		if dash >= 0 && dash != 1 { return usage("pmux claude requires exactly one model ID before --") }
		if hasClientModelArg(args[1:]) { return usage("Client arguments must not contain '--model'; use 'pmux launch --model <id>'.") }
...
func doctorCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		if len(args) != 0 {
			if !cmd.Flags().Changed("fix") { return usage("doctor accepts positional IDs only after --fix") }
			fixes = append(fixes, args...)
		}
		if cmd.Flags().Changed("fix") && !d.interactive() && !d.flags.Yes { return usage("Noninteractive operation requires --yes; no changes were made.") }
...
func serviceCommand(d dispatcher) *cobra.Command {
	...
	parent.RunE = func(cmd *cobra.Command, _ []string) error { op := app.OpServiceStatus; if d.interactive() { op = app.OpTUIService }; return d.run(cmd, op, nil, nil) }
...
func serviceTimedCommand(d dispatcher, action string) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { return d.run(cmd, app.Operation("service."+action), nil, opts("timeout", timeout)) }
	return cmd
}
...
func serviceInstallCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { return d.run(cmd, "service.install", nil, opts("start", start)) }
	return cmd
}
...
func serviceLogsCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error {
		if lines < 0 { return usage("--lines must not be negative") }
		if clear != "" {
...
func configCommand(d dispatcher) *cobra.Command {
	...
	validScope := func() error { if scope != "proxy" && scope != "pmux" { return usage("--scope must be proxy or pmux") }; return nil }
	parent.RunE = func(cmd *cobra.Command, _ []string) error {
		if err := validScope(); err != nil { return err }
		if d.interactive() { return d.run(cmd, app.OpTUIConfig, nil, opts("scope", scope)) }
...
func configShowCommand(d dispatcher, scope *string, valid func() error) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { if err := valid(); err != nil { return err }; return d.run(cmd, "config.show", nil, opts("scope", *scope, "effective", effective, "reveal_paths", reveal)) }
	return cmd
}
...
func configGetCommand(d dispatcher, scope *string, valid func() error) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { if err := valid(); err != nil { return err }; return d.run(cmd, "config.get", args, opts("scope", *scope)) }
	return cmd
}
...
func configSetCommand(d dispatcher, scope *string, valid func() error) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { if err := valid(); err != nil { return err }; return d.run(cmd, "config.set", args, opts("scope", *scope, "restart", restart)) }
	return cmd
}

func configEditCommand(d dispatcher, scope *string, valid func() error) *cobra.Command {
	var editor string; var restart bool
	...
	cmd.Flags().BoolVar(&restart, "restart", false, "restart after a restart-required change")
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { if err := valid(); err != nil { return err }; if !d.interactive() { return usage("This operation requires an interactive terminal; use `pmux config set` instead.") }; return d.run(cmd, "config.edit", nil, opts("scope", *scope, "editor", editor, "restart", restart)) }
	return cmd
}
...
func configBackupCommand(d dispatcher, scope *string, valid func() error) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { if err := valid(); err != nil { return err }; return d.run(cmd, "config.backup", nil, opts("scope", *scope)) }
	return cmd
}
...
func configRestoreCommand(d dispatcher, scope *string, valid func() error) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error { if err := valid(); err != nil { return err }; return d.run(cmd, "config.restore", args, opts("scope", *scope, "restart", restart)) }
	return cmd
}
...
func updateCommand(d dispatcher) *cobra.Command {
	...
	parent.RunE = func(cmd *cobra.Command, _ []string) error { return d.run(cmd, "update.check", nil, opts("component", "all")) }
...
func updateCheckCommand(d dispatcher) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { if component != "all" && component != "self" && component != "proxy" { return usage("--component must be all, self, or proxy") }; return d.run(cmd, "update.check", nil, opts("component", component)) }
	return cmd
}
...
func updateComponentCommand(d dispatcher, component string) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error { return d.run(cmd, app.Operation("update."+component), nil, opts("version", target)) }
	return cmd
}

func opts(values ...any) map[string]any {
	if len(values)%2 != 0 { panic("opts requires key/value pairs") }
	out := make(map[string]any, len(values)/2)
	for i := 0; i < len(values); i += 2 { out[values[i].(string)] = values[i+1] }
	return out
}

func hasClientModelArg(arguments []string) bool {
	for _, argument := range arguments {
		if argument == "--model" || strings.HasPrefix(argument, "--model=") { return true }
	}
	return false
}
